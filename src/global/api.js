...
import { replaceHtml, getObjType, chatatABC, luckysheetactiveCell,generateRandomId } from "../utils/util";
 ...
 * @returns {Array}
 */
export function getHasDataRangeAxis(colIndex) {
    if (!colIndex || colIndex < 0) return "";
    let result = "";
    let sheetIndex = Store.currentSheetIndex;
    for (let i = Store.flowdata.length - 1; i >= 0; i--) {
        if (Store.flowdata[i].some(cell => cell.hasOwnProperty('m') && cell.m)) {
            result = getRangetxt(sheetIndex, { column: [0,colIndex], row: [0,i] });
            break;
        }
    }
    return result;
