...
function luckysheetextendtable(type, index, value, direction, sheetIndex) {
    ...
    if (type == "row") {
        ...
        for (let c = 0; c < d[0].length; c++) {
            row.push({id:generateRandomId("cell")});
        }
        ...
        // *添加空列模板这里请保持为push id:xxxx;
        let col = [];
        for (let r = 0; r < d.length; r++) {
            col.push({id:generateRandomId("cell")});
        }
    ...
    if (type == "row") {
        ...
        for (let i = st; i <= ed; i++) {
            for (let j = 0; j < d[i].length; j++) {
                if(d[i][j]!=null && d[i][j].hasOwnProperty("imgId") && d[i][j].imgId){
                    imgCtrl.removeImgItem(d[i][j].imgId);
                }
            }
        }
