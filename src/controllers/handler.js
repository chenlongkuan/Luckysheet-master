...
export default function luckysheetHandler() {
            ...
            for (let r = 0; r < data.length; r++) {
                for (let c = 0; c < data[0].length; c++) {
                    ...
                    if (data[r] != null && data[r][c] != null) {
                        ...
                        if (value.imgId) {
                            //插入图片到指定行列
                            imageCtrl.pastImgToCell(value.imgId, r + row_s, c + col_s);
                            //移除原位置的图片
                            imageCtrl.removeImgItem(value.imgId);
                        }
                    }
                    ...
                    let targetCell = d[r + row_s][c + col_s];
                    // 创建一个不包含id的cell副本
                    let cellWithoutId = $.extend({}, value);
    ...
    $("#luckysheet-imgUpload").on("change", function (e) {
        ...
        // 重置 input 的值，以便下次选择相同文件时也能触发 change 事件
        $(this).val('');
    });
