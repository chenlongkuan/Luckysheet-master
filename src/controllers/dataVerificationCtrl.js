...
const dataVerificationCtrl = {
    ...
    dropdownListShow: function () {
        ...
        optionHtml += '</div>'; // 关闭包裹列表的div

        let $dropdownList = $("#luckysheet-dataVerification-dropdown-List");
        $dropdownList
            .html(optionHtml)
            ...
            .css({
                ...
                'overflow-y': 'auto', // 设置内滚动
                'text-align': 'left'  // 设置文字左对齐
            });

        // 监控单元格内容变化并过滤
        $("#luckysheet-rich-text-editor").off('input.dropdownFilter').on('input.dropdownFilter', function () {
            let filterText = $(this).text().toLowerCase();
            $("#luckysheet-dataVerification-dropdown-List .luckysheet-dropdown-list-content .dropdown-List-item").each(function () {
                let itemText = $(this).text().toLowerCase();
                $(this).toggle(itemText.indexOf(filterText) > -1);
            });
        });
        
        // 使用事件委托处理鼠标滚轮事件
        $dropdownList.on('mouseenter', function () {
            // 鼠标进入下拉列表区域时，给外层元素绑定mousewheel事件代理
            $dropdownList.closest("#luckysheet-grid-window-1").on('mousewheel DOMMouseScroll', '.luckysheet-dropdown-list-content', function (event) {
                if (event.originalEvent.wheelDelta > 0 || event.originalEvent.detail < 0) {
                    // 向上滚动
                    $dropdownList.scrollTop($dropdownList.scrollTop() - 20);
                } else {
                    // 向下滚动
                    $dropdownList.scrollTop($dropdownList.scrollTop() + 20);
                }
                event.preventDefault(); // 阻止默认滚动行为
                event.stopPropagation(); // 阻止事件冒泡，阻止事件向上传播
            });
        }).on('mouseleave', function () {
            // 鼠标离开下拉列表区域时，移除外层元素上的事件代理
            $dropdownList.closest("#luckysheet-grid-window-1").off('mousewheel DOMMouseScroll', '.luckysheet-dropdown-list-content');
        });
        let myh = $dropdownList.outerHeight();
        let currentWinH = $("#luckysheet-cell-main")[0].scrollHeight;

        if (row + myh > currentWinH - 42 - 6) {
            $dropdownList.css({ 'top': row_pre - myh })
        }
