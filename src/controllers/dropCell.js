...
import { generateRandomId, getObjType, replaceHtml } from '../utils/util';
import Store from '../store';
import locale from '../locale/locale';
import dayjs from 'dayjs'
import imgCtrl from '../controllers/imageCtrl'
    ...
    createIcon: function () {
        ...
        $("#luckysheet-dropCell-icon").mouseover(function () {
            ...
            const locale_dropCell = _locale.dropCell;
             $("#" + Store.container).after(replaceHtml(_this.typeListHtml, {
            ...
            for (let i = apply_str_c; i <= apply_end_c; i++) {
                ...
                if (direction == "down") {
                    for (let j = apply_str_r; j <= apply_end_r; j++) {
                        ...
                        //有图片的单元格，其值不变
                        if (cell.imgId) {
                            let imgSrc = imgCtrl.images[cell.imgId].src;
                            let newImgId = generateRandomId();
                            //复制图片到新的单元格
                            imgCtrl._insertImg(imgSrc, j, i, newImgId);
                        }

                        // 使用$.extend赋值除id以外的所有属性
                        let targetCell = d[j][i];
                        // 创建一个不包含id的cell副本
                        let cellWithoutId = $.extend({}, cell);
                        delete cellWithoutId.id;
                        // 合并属性
                        $.extend(targetCell, cellWithoutId);
                ...
                if (direction == "right") {
                    for (let j = apply_str_c; j <= apply_end_c; j++) {
                        ...
                        if (cell.imgId) {
                            ...
                            let newImgId = generateRandomId();
                            //复制图片到新的单元格
                            imgCtrl._insertImg(imgSrc, i, j, newImgId);
                        }
                        // 使用$.extend赋值除id以外的所有属性
                        let targetCell = d[i][j];
                        // 创建一个不包含id的cell副本
                        let cellWithoutId = $.extend({}, cell);
                        delete cellWithoutId.id;
                        // 合并属性
